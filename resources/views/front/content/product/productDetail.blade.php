...
@section('banner')
    <div class="pt-12 bg-[#f7f7f7]">
        <div class="container mx-auto px-2 xl:px-36">
...
@section('content')
    <div class="container mx-auto px-2 xl:px-36 text-lg">
        <div class="w-full py-8 md:py-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-24 mt-7 md:px-16">
                <div class="col-span-1 md:col-span-2">
                    <div id="indicators-carousel" class="relative w-full" data-carousel="static">
                        <!-- Carousel wrapper -->
                        <div class="relative h-56 overflow-hidden rounded-lg md:h-[450px] border border-gray-300">
                            <!-- Item 1 -->
                        ...
                        <div class="grid grid-cols-4 gap-5 mt-10">
                            @foreach($product->images as $image)
                                <button type="button" aria-current="true" aria-label="Slide {{$loop->index}}" data-carousel-slide-to="{{$loop->index}}"
                                        class="shadow border border-gray-300 rounded-lg hover:border-blue-400 overflow-hidden">
                                    <img src="{{asset($image)}}" class="w-full h-36 rounded-lg hover:scale-110" alt="product-image">
                                </button>
                            @endforeach
                        </div>
                    </div>
            ...
            <!-- Product detail -->
            <div class="border-b border-gray-300 dark:border-gray-700 mt-4 mt-5 md:mt-16">
...
{{--                            </div>--}}
{{--                        </div>--}}
                        <div class="w-full md:w-[65%] flex flex-col md:pl-20     gap-4 md:gap-10">
                            @foreach($product->numbers as $number)
                ...
                <div class="hidden py-12 rounded-lg flex flex-col items-center" id="dashboard" role="tabpanel" aria-labelledby="component-tab">
                    <div class="md:flex md:justify-center grid grid-cols-2 gap-8 items-center">
                        @foreach($product->components as $component)
                            <div class="h-56">
                                <div class="w-32 md:w-40 h-32 md:h-40 rounded-full border-2 border-blue-300">
                                    <img src="{{asset($component->image)}}" alt="{{Str::slug($component->name)}}" class="w-full h-full rounded-full">
                                </div>
                                <h5 class="text-center mt-2 font-semibold w-36 md:w-40">
                                    {{$component->name}}
                                </h5>
                            </div>
                        @endforeach
                    ...
                    <div class="bg-gray-100 mt-8 p-8 hidden md:block">
                        <ul>
                            @foreach($product->components as $component)
                                <li>
                                    <div class=" mt-2">
                ...
                <div class="hidden py-12 rounded-lg" id="settings" role="tabpanel" aria-labelledby="settings-tab">
                    <!-- assign -->
                    <div class="my-2 pl-5 md:px-32 md:my-8">
                        <ul class="text-black text-base md:text-lg list-disc flex flex-col gap-3">
                            <li>Cấp ẩm, giúp da căng bóng mềm mịn</li>
                            <li>Kích thích tăng sinh tế bào mới, thúc đẩy quá trình turn-over trên da</li>
                            <li>Cải thiện kết cấu và tone da</li>
                            <li>Giảm sự nhạy cảm của da, làm dịu da</li>
                            <li>Hỗ trợ chống lão hóa, chống oxy hóa</li>
                        </ul>
                    </div>
                </div>

                <div class="hidden py-16 rounded-lg md:px-32 font-medium text-black text-base " id="contacts" role="tabpanel" aria-labelledby="contacts-tab">
                    <h4 class="uppercase">Làm sạch da:</h4>
                    <ul class="font-normal list-disc pl-7 flex flex-col gap-2">
                    ...
                    <h4 class="uppercase mt-4">Thoa kem dưỡng ẩm:</h4>
                    <ul class="font-normal list-disc pl-7 flex flex-col gap-2">
                    ...
                    <h4 class="uppercase mt-4">Sử dụng hàng ngày:</h4>
                    <ul class="font-normal list-disc pl-7 flex flex-col gap-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-6 gap-6">
                    @foreach($relatedProducts as $item)
                        <div class="h-96">
                            ...
                            <div class="py-2 flex flex-col justify-between h-40">
                                <div>
                                    <h4 class="font-semibold text-base md:text-lg"><a href="{{route('product.detail', $item->slug)}}">{{$item->name}}</a></h4>
                                    <h5 class="text-sm md:text-base"><a href="{{route('product.detail', $item->slug)}}">{{$item->title}}</a></h5>
                                </div>
                                <div class="mt-4">
